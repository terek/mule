...
Music = function(args, keyboard) {
    this.bassCount_ = parseInt(args['maxBass']) - parseInt(args['minBass']) + 1;
    this.trebleCount_ = parseInt(args['maxTreble']) - parseInt(args['minTreble']) + 1;
...
Music.prototype.reset = function() {
    ...
    do {
        this.dice_.push(...randomSample(sequence, args['iterations']));
    } while (this.dice_.length < args['iterations']);
    this.index_ = 0;
}
...
Music.prototype.next = function() {
    ...
    var i = (clef == 0 ?
        parseInt(args['minBass']) + dice :
        parseInt(args['minTreble']) + dice - this.bassCount_);
    var note = i % 7;
    var octave = (i - note) / 7;
